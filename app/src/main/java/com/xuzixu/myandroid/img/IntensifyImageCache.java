...
class IntensifyImageCache extends IntensifyCache<Integer, IntensifyImageCache.ImageCache, Void> {
    ...
    @Override
    protected void entryRemoved(boolean evicted, Integer key,IntensifyImageCache.ImageCache oldValue, IntensifyImageCache.ImageCache newValue) {
        if (oldValue != null) oldValue.evictAll();
    }

    @Override
    protected IntensifyImageCache.ImageCache create(Integer key) {
        return new IntensifyImageCache.ImageCache(mSubMaxSize, key);
    }
