...
class IntensifyImageDelegate {
    ...
    private IntensifyImageDelegate.Callback mCallback;

    private DisplayMetrics mDisplayMetrics;

    private IntensifyImageDelegate.IntensifyImageHandler mHandler;

    private IntensifyImageDelegate.Image mImage;
    ...
    private volatile IntensifyImageDelegate.State mState = IntensifyImageDelegate.State.NONE;
    ...
    public IntensifyImageDelegate(DisplayMetrics metrics, IntensifyImageDelegate.Callback callback) {
        mDisplayMetrics = metrics;
        mCallback = Utils.requireNonNull(callback);
        HandlerThread handlerThread = new HandlerThread(TAG);
        handlerThread.start();
        mHandler = new IntensifyImageDelegate.IntensifyImageHandler(handlerThread.getLooper());
        ...
        mZoomAnimator.addUpdateListener(new IntensifyImageDelegate.ZoomAnimatorAdapter());
    }
    ...
    public void load(String path) {
        load(new IntensifyImageDelegate.ImagePathDecoder(path));
    }

    public void load(File file) {
        load(new IntensifyImageDelegate.ImageFileDecoder(file));
    }

    public void load(InputStream inputStream) {
        load(new IntensifyImageDelegate.ImageInputStreamDecoder(inputStream));
    }

    public void load(IntensifyImageDelegate.ImageDecoder decoder) {
    ...
    //@WorkerThread
    private void prepare(IntensifyImageDelegate.ImageDecoder decoder) {
        mImage = new IntensifyImageDelegate.Image(decoder);
        mImageArea.setEmpty();
        mState = IntensifyImageDelegate.State.SRC;
        load();
    }
    ...
    private void load() {
        ...
        mState = IntensifyImageDelegate.State.LOAD;
    }
    ...
    private void initialize(Rect drawingRect) {
        ...
        mState = IntensifyImageDelegate.State.INIT;
    ...
    private void initScaleType(Rect drawingRect) {
        ...
        mNeedReset = true;
        mState = IntensifyImageDelegate.State.FREE;
    }
    ...
    private void prepareDraw(Rect rect) {
        ...
        if (mImage.mImageSampleSize > sampleSize) {
            ...
            List<IntensifyImageDelegate.ImageDrawable> drawables = new ArrayList<>();
            ...
            if (imageCache != null) {
                for (int i = blocks.top; i <= blocks.bottom; i++) {
                    for (int j = blocks.left; j <= blocks.right; j++) {
                        ...
                        drawables.add(new IntensifyImageDelegate.ImageDrawable(bitmap, src, dst));
                    }
                }
            }
    ...
    private void release() {
        ...
        mState = IntensifyImageDelegate.State.NONE;
    }
    ...
    public void setScale(float scale) {
        ...
        mTempScale = scale;
        mNeedReset = false;
        if (mState.ordinal() > IntensifyImageDelegate.State.INIT.ordinal()) {
            mState = IntensifyImageDelegate.State.INIT;
            requestInvalidate();
    ...
    public void zoomScale(Rect drawingRect, float scale, float focusX, float focusY) {
        if (mState.ordinal() < IntensifyImageDelegate.State.FREE.ordinal() || Utils.isEmpty(drawingRect))
            return;
        mZoomAnimator.cancel();
        mStartRect.set(mImageArea);
    ...
    public List<IntensifyImageDelegate.ImageDrawable> obtainImageDrawables(Rect drawingRect) {
        if (Utils.isEmpty(drawingRect) || isNeedPrepare(drawingRect)) {
            return Collections.emptyList();
        }

        ArrayList<IntensifyImageDelegate.ImageDrawable> drawables = obtainBaseDrawables();
    ...
    public ArrayList<IntensifyImageDelegate.ImageDrawable> obtainBaseDrawables() {
        ArrayList<IntensifyImageDelegate.ImageDrawable> drawables = new ArrayList<>();
        drawables.add(new IntensifyImageDelegate.ImageDrawable(mImage.mImageCache,
    ...
    private class Image {
        ...
        private Image(IntensifyImageDelegate.ImageDecoder decoder) {
            try {
    ...
    public static class ImagePathDecoder implements IntensifyImageDelegate.ImageDecoder {
    ...
    public static class ImageFileDecoder implements IntensifyImageDelegate.ImageDecoder {
        private File mFile;
    ...
    public static class ImageInputStreamDecoder implements IntensifyImageDelegate.ImageDecoder {
    ...
    private class IntensifyImageHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_IMAGE_SRC:
                    prepare((IntensifyImageDelegate.ImageDecoder) msg.obj);
                    requestInvalidate();
                    break;
